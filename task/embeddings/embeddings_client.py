    ...
    def get_embeddings(
        ...
        if response.status_code == 200:
            ...
            if not isinstance(data, list):
                raise ValueError("Unexpected embeddings response format: 'data' is not a list")
            return self._from_data(data)
        raise RuntimeError(f"Embeddings request failed (HTTP {response.status_code}): {response.text}")
